    ...
    loop {
        ...
        if client.is_push_paused_for_auth().await {
            ...
            if let Some(refresh_msg) = client.initiate_token_refresh().await {
                let Some(frame) =
                    SyncFrame::try_encode(SyncMessageType::TokenRefresh, &refresh_msg)
                else {
                    tracing::error!("failed to encode reactive token refresh frame; skipping");
                    return;
                };
        ...
        if let Some(resync) = client.take_pending_resync().await {
            let Some(frame) = SyncFrame::try_encode(SyncMessageType::ResyncRequest, &resync) else {
                tracing::error!("failed to encode resync request frame; skipping");
                return;
            };
            let mut sink_guard = sink.lock().await;
            if let Err(e) = sink_guard
    ...
    S::Error: std::fmt::Display,
{
    ...
    loop {
        ...
        {
            let Some(frame) = SyncFrame::try_encode(SyncMessageType::TokenRefresh, &refresh_msg)
            else {
                tracing::error!("failed to encode token refresh frame; skipping");
                return;
            };
            let mut sink_guard = sink.lock().await;
            if let Err(e) = sink_guard
...
mod tests {
    ...
    async fn dispatch_delta_ack() {
        ...
        let frame =
            SyncFrame::try_encode(SyncMessageType::DeltaAck, &ack).expect("test frame encode");
    ...
    async fn dispatch_delta_reject() {
        ...
        let frame = SyncFrame::try_encode(SyncMessageType::DeltaReject, &reject)

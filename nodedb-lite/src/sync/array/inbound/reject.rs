...
use nodedb_types::sync::wire::array::ArrayRejectMsg;
...
impl<S: StorageEngineSync> ArrayInbound<S> {
    ...
    /// rejection via `tracing::warn!`. Returns
    ...
    /// ack).
    pub fn handle_reject(&self, msg: &ArrayRejectMsg) -> Result<InboundOutcome, LiteError> {
        let op_hlc = Hlc::from_bytes(&msg.op_hlc_bytes);
        let was_present = self.pending.remove(op_hlc)?;
...
mod tests {
    ...
    #[test]
    fn handle_reject_drops_op_from_pending() {
        let (inbound, _schemas, pending, _storage) = make_inbound();

        // Pre-populate pending with an op at HLC(10).
        let op_hlc = hlc(10);
        let op = put_op("any", 10, 1);
        pending.enqueue(&op).unwrap();
        ...
        let outcome = inbound.handle_reject(&msg).unwrap();
        assert_eq!(outcome, InboundOutcome::RejectAcknowledged);
        assert_eq!(pending.len().unwrap(), 0, "op must be removed from pending");
    }
}

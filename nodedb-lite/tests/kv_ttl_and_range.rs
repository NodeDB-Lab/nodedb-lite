...
async fn ttl_expires_on_read() {
    ...
    let got = db.kv_get(col, "k").await.expect("kv_get immediate");
    assert_eq!(
        got.as_deref(),
    ...
    let got = db.kv_get(col, "k").await.expect("kv_get after expiry");
...
async fn range_scan_lex_order() {
    ...
    db.kv_put(col, "a", b"va").await.expect("put a");
    db.kv_put(col, "c", b"vc").await.expect("put c");
    db.kv_put(col, "b", b"vb").await.expect("put b");
    db.kv_flush().await.expect("flush");

    let results = db
        .kv_range_scan(col, None, None, None)
        .await
        .expect("range_scan");
...
async fn range_scan_bounds() {
    ...
    for ch in b'a'..=b'e' {
        ...
        db.kv_put(col, &key, val.as_bytes()).await.expect("put");
    }
    db.kv_flush().await.expect("flush");

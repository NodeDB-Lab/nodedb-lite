...
async fn erase_tombstone_removes_cell() {
    let harness = common::SyncHarness::new_in_memory().await;
    harness.create_array("gdpr").await;
    ...
    harness.flush("gdpr").await;

    // Verify cell is visible before erase.
    let before = harness.read_coord("gdpr", 2, i64::MAX).await;
    assert_eq!(
        before,
    ...
    let after = harness.read_coord("gdpr", 2, i64::MAX).await;
    assert!(
        after.is_none(),
...
#[tokio::test(flavor = "multi_thread")]
async fn cross_replica_erase_removes_cell() {
    let harness = common::SyncHarness::new_in_memory().await;
    harness.create_array("xr_gdpr").await;
    ...
    harness.deliver(&put);
    harness.flush("xr_gdpr").await;

    let before = harness.read_coord("xr_gdpr", 9, i64::MAX).await;
    assert!(
        before.is_some(),
    ...
    let after = harness.read_coord("xr_gdpr", 9, i64::MAX).await;
    assert!(
        after.is_none(),
...
#[tokio::test(flavor = "multi_thread")]
async fn erase_nonexistent_coord_is_safe() {
    let harness = common::SyncHarness::new_in_memory().await;
    harness.create_array("no_cell").await;
    ...
    harness.flush("no_cell").await;

    let val = harness.read_coord("no_cell", 99, i64::MAX).await;
